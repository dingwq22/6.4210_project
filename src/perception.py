...
def grasp_selection(plant, diagram, station, meshcat):
  system = diagram
  ...
  for i in range(3):
    ...
    pcd.append(
        cloud.Crop(lower_xyz=[-0.3, -0.3, 0], upper_xyz=[0.3, 0.3, 0.3])
    )
  ...
  for i in range(100 if running_as_notebook else 50):
      cost, X_G = GenerateAntipodalGraspCandidate(
          # station,
          # context,
          internal_model,
