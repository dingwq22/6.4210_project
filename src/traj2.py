...
class GridProblem(PathCostProblem):
    ...
    def step(self, state, action):
        ...
        if action == 'left':
            ...
            dr, dc = self.grid_dir_to_delta[d]
            new_r, new_c = r + dr, c + dc
            if (new_r, new_c) in self.goal:
               id = self.goal.index((new_r, new_c))
               carrying = tuple(carrying[i] if i!=id else True for i in range(len(carrying)))         
    ...
    def h(self, state):
        """Manhattan distance."""
        return sum([abs(state[0] - g[0]) + abs(state[1] - g[1]) for g in self.goal])
